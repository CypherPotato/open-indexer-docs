...
A chat client provides a user interface via an [AI Gateway](/docs/en/inference/ai-gateway) that allows the user to converse with their assistant. A chat client is integrated with the AI gateway inference and supports deep thinking, search, text conversation, and image sending. Audio features depend on the integration and client configuration.

You can customize your chat client’s interface with CSS and custom JavaScript, and also choose the language of the chat resources.
...
A chat session expires after some time for security of the generated access token. When you call this endpoint with a `tag`, you can call the same endpoint multiple times and obtain the chat session that is active for the given tag, or create a new chat if no session is in progress.
...
If a session is close to expiring, it is renewed for an additional 20 minutes on the user’s next message.

For details on how to create a chat session, see the [Create Web Chat Session](https://inference.aivax.net/apidocs#CreateWebChatSession) endpoint.

When creating a session, use additional context for information that helps the assistant in that conversation but should not become permanent memory: displayed name, client plan, preferred language, referrer page, product the user is viewing, order number, or current flow state. Do not use this field for secrets, internal tokens, or data the model should not see. The additional context goes into inference and can influence responses, tools, and workers.

The web chat accepts text messages and attachments. Images are forwarded as multimodal content when the model or pipeline supports it. Audio can be sent as multimodal input in compatible clients and can also be synthesized as a response when the chat client’s audio synthesis setting is active. Files and videos depend on the client type, chosen model, and multimodal processing configured in the gateway. When the channel does not support an attachment, the integration should transform the event into a textual message indicating that the user sent unsupported content, so that the assistant replies clearly.

## Integration sessions

AIVAX provides integrations for chat clients via Telegram and WhatsApp, including [Z-Api](https://www.z-api.io/), Evolution API, and Kapso. Each conversation in an app is an individual session, identified by the conversation ID or the user’s phone number.
...
For production, configure limits on the chat client before opening the channel to the public. Message limits per hour, maximum messages in history, and session duration help control cost, abuse, and context growth. Enable debug commands only for internal testing. If the assistant uses memory, explain to the user when information may be remembered and avoid storing sensitive data. If using workers, use the `externalUserId` to apply blocks, subscription rules, profile enrichment, or auditing without embedding that logic in the model instructions.
