...
Skills are primarily provided through a name and a brief description of what that skill is and when it should be used. You can have multiple skills in your account, but use only some of them in your [AI gateway](/docs/en/entities/ai-gateways/ai-gateway). The skills enabled in the AI gateway are injected into the system instructions of your model, and an additional function is added to the context.

When the model calls the skill‑reading tool, the instructions for that skill are inserted into the system instructions of the context dynamically. Only one skill can be active per context, and the LLM can change the active skill as needed.
...
- **Too few or too many skills**: Ideally have 3–10 skills. Fewer than that, consider using base instructions. More than that can confuse the model.
...
| Context change | Yes, can switch | No, fixed |
...
- **Monitor usage**: Check whether the model is activating skills correctly.
