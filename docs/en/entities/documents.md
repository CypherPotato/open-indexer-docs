...
- Do not create documents that are too short (with 10 words or fewer).
- Do not create documents that are too large (with 700 words or more).
...
- Restrict the document’s content to talk about only one topic or subject.
- Use human, simple, and easy‑to‑understand language.

## API Usage

Since all documents are entities that belong to a [collection](/docs/en/entities/rag), always have the collection where the document is/will be located at hand.
