...
| `add-protocol-tool` | Adiciona uma definição de [protocol function](/docs/pt-br/tools/protocol-functions) ao contexto. | `tool`: objeto definição da protocol function|
...
O exemplo abaixo ilustra um [Cloudflare Worker](https://workers.cloudflare.com/) que autentica uma conversa no Telegram com base no ID do usuário:

```js
export default {
  async fetch(request, env, ctx) {
    const CHECKING_GATEWAY_ID = "0197dda5-985f-7c76-96e5-0d0451c596e5";
    const ALLOWED_TELEGRAM_USER_IDS = ["123456789"];
    ...
    if (request.method == "POST") {
      ...
      if (gatewayId === CHECKING_GATEWAY_ID &&
        ...
        const telegramUserId = event.data.externalUserId.split(':')[1]?.split('@')[0];

        if (!telegramUserId || !ALLOWED_TELEGRAM_USER_IDS.includes(telegramUserId)) {
