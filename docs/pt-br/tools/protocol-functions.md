...
Se você tem muitas ferramentas, ferramentas dinâmicas ou um sistema que já implementa Model Context Protocol, prefira [MCP](/docs/pt-br/tools/mcp). Se você quer usar capacidades já mantidas pela AIVAX, prefira [ferramentas embutidas](/docs/pt-br/tools/builtin-tools). Se você precisa tomar decisões antes ou depois de eventos da inferência, prefira [workers](/docs/pt-br/inference/workers). Funções de protocolo ficam no meio: são mais simples que MCP e mais específicas que workers, mas ainda dão ao modelo uma ferramenta controlada para executar uma ação externa.
...
Essas funções são definidas no [AI Gateway](/docs/pt-br/inference/ai-gateway), o que permite a criação de agentes inteligentes que realizam ações sem intervenção humana. Elas seguem uma sintaxe simples, esperam o nome da função, uma descrição do que ela faz e os parâmetros de invocação.
...
Veja o manual de [autenticação](/docs/pt-br/authentication) para entender como autenticar requisições reversas do AIVAX.

#### Autenticação de usuário

As chamadas de função enviam um campo `$.context.externalUserId` contendo a tag de usuário criada em uma [sessão de chat](/docs/pt-br/features/chat-clients). Essa tag pode ser usada para autenticar o usuário que chamou essa função.

#### Considerações de segurança

Para o modelo de IA, somente é visível o nome, descrição e formato da função. Ela não é capaz de ver o endpoint para onde essa função aponta. Além disso, ela não possui acesso à tag do usuário que está autenticado em um [cliente de chat](/docs/pt-br/features/chat-clients).

## Funções especialistas

Além das [funções embutidas](/docs/pt-br/tools/builtin-tools), você pode definir funções especialistas, que executam tarefas específicas na sua conta da AIVAX.
