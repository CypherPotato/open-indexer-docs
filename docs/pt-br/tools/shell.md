...
Na linha acima, `get-users` é uma ferramenta personalizada que retorna uma lista de usuários em formato CSV. O comando `grep` filtra os resultados para encontrar "John Doe", e o `awk` extrai e formata as colunas desejadas. Essa ferramenta pode ter sido definida por [MCP](/docs/pt-br/tools/mcp), [ferramentas embutidas](/docs/pt-br/tools/builtin-tools) ou ser uma [ferramenta de protocolo](/docs/pt-br/tools/protocol-functions).
